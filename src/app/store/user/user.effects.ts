...
type Action33 = fromActions.All33;
...
@Effect()
signInEmail: Observable<Action33> =
  this.actions
    .pipe(
...
@Effect()
signUpEmail: Observable<Action33>
   = this.actions.pipe(
                    ofType(fromActions.Types.SIGN_UP_EMAIL),
                    map((action: fromActions.SignUpEmail) => action.credentials),
                    switchMap( credentials =>
                                           from (this.afAuth.auth.createUserWithEmailAndPassword(credentials.email, credentials.password))
                                           .pipe(
                                                tap(() => {
